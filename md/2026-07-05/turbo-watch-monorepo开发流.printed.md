...
- **真因**:**"保存文件"这个动作本身,几乎从来不是单个文件系统事件**——
  ...
  - 编辑器保存同理:无论"写临时文件+改名"还是"写回原文件",一次"保存"往往拆成多个文件事件。
...
### 连带两个工程概念(含一次被数据打脸的纠正)
- **原子保存(atomic save)**:为避免别人读到"写了一半"的残缺文件,一种做法是**先把新内容完整写到临时文件,再用 `rename()` 一步顶替原文件**。`rename()` 是原子操作(要么全成、要么没发生),所以文件名任何时刻都指向完整内容。**代价:产生多个文件事件;且因为是"新文件顶替旧文件",inode 会变。**
- **⚠️ 但不是所有编辑器都用 rename 式**:用 `ls -i <文件>`(inode = 文件在磁盘的"身份证号")就能判别——rename 顶替式保存后 inode **会变**;就地写 / 写回原文件则 inode **不变**。
- **实测(2026-07-05)**:本机 **WebStorm 连续保存,inode 恒为 `29515180` 不变** → 它**不是** rename 顶替式,而是把内容**写回原文件**(JetBrains 倾向保留 inode / 权限 / 硬链接);python 的 `open('w')` 同样保留 inode。→ **纠正**:调查中我一度口断"WebStorm=写临时文件+改名",被 inode 实验证伪。但**双构建的根因(一次保存=多个文件事件)不受影响**——多事件不必来自 rename。
- **去抖 debounce**:成熟的文件监听器侦测到变化后等几十毫秒,把一簇事件**合并成一次**再触发(vite 成熟,`turbo watch` 较弱,所以撞见了多跑一次)。
